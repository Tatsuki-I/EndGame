...
rookatk     :: BitBoards -- ^ Rook Pos
            -> BitBoards -- ^ Blockers
            -> ((Int, Int, Int, Int), Word16, (Word8, Word8, Word8, Word8))
            -- -> BitBoards -- ^ Attack
rookatk r b =  (nwes, fromIntegral pexted, g nwes $ fromIntegral pexted)
               where m = rmask V.! rkpos
                     rkpos = head $ bitcheck r
                     pexted = pext b m
                     nwes = rpextmask V.! rkpos

g                 :: (Int, Int, Int, Int) -> Word16 -> (Word8, Word8, Word8, Word8)
g (n, w, e, s) bb =  ( ratkls V.! (fromIntegral $ fromIntegral (shiftR bb (w + e + s)) .&. lnMask n)
                     , ratkls V.! (fromIntegral $ fromIntegral (shiftR bb (e + s)    ) .&. lnMask w)
                     , ratkls V.! (fromIntegral $ fromIntegral (shiftR bb s          ) .&. lnMask e)
                     , ratkls V.! (fromIntegral $ fromIntegral                     bb  .&. lnMask s) )
                     where getLineAtk ln bb ls = bb .|. lnMask ln
                           lnMask    :: Int -> Word8
                           lnMask  0 = 0b00000000
                           lnMask  1 = 0b00000001
                           lnMask  2 = 0b00000011
...
rpextnmask :: V.Vector BitBoards
rpextnmask =  V.fromList [ 0x0000000000000000
                         , 0x0000000000000000
                         , 0x0000000000000000
                         , 0x0000000000000000
                         , 0x0000000000000000
...
rpextmask :: V.Vector (Int, Int, Int, Int)
rpextmask =  V.fromList [ (6, 0, 6, 0) -- a1
                        , (6, 0, 5, 0) -- b1
                        , (6, 1, 4, 0) -- c1
                        , (6, 2, 3, 0) -- d1
                        , (6, 3, 2, 0) -- e1
...
ratkls :: V.Vector Word8
ratkls =  V.fromList [ 0b1111111
                     , 0b1111110
                     , 0b1111100
                     , 0b1111100
                     , 0b1111000
...
rmagics :: V.Vector BitBoards
rmagics =  V.fromList [ 0x0080001020400080, 0x0040001000200040, 0x0080081000200080, 0x0080040800100080
...
rmask :: V.Vector BitBoards
rmask =  V.fromList [ 0x000101010101017E, 0x000202020202027C, 0x000404040404047A, 0x0008080808080876
